...
We can rewrite the LLRT by defining the test statistic $ T_k= \mathbf{x}_k^T \mathbf{s} $    
and target signal energy $\displaystyle E = \sum_{n = kW}^{(k+1)W}s[n]^2$. If we implement an ML detector, $\eta=1 \implies \log( \eta)=0$. Then, the LLRT becomes
